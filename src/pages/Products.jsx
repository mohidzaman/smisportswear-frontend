...
const Products = () => {
  ...
  return (
    <motion.div 
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="flex flex-col bg-background-dark min-h-screen pt-24 overflow-x-hidden"
    >
      <section className="section-elite relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 w-full">
          <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-32 gap-12">
            ...
            >
              <button className="group flex items-center space-x-6 px-10 py-5 bg-white/[0.02] border border-white/10 rounded-[2.5rem] text-[10px] font-black tracking-[0.3em] uppercase hover:bg-white hover:text-black transition-all duration-700 shadow-premium">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
            {categories.map((cat, i) => (
              <motion.div 
                ...
                className="group relative rounded-[4rem] overflow-hidden border border-white/5 block hover-card-elite h-[700px]"
                whileHover={{ y: -15 }}
              >
                <img 
                  src={cat.img} 
                ...
                <div className="absolute inset-0 flex flex-col justify-end p-16 z-10">
                  <h3 className="text-5xl md:text-6xl font-black text-white uppercase italic tracking-tighter leading-none mb-10">{cat.title}</h3>
                  ...
                  <div className="max-h-0 group-hover:max-h-[300px] opacity-0 group-hover:opacity-100 transition-all duration-1000 ease-apple overflow-hidden">
                    <p className="text-white/40 text-sm leading-relaxed font-bold uppercase tracking-widest italic mb-10 max-w-sm">{cat.desc}</p>
                    <ul className="grid grid-cols-2 gap-x-8 gap-y-4 mb-12">
                      {cat.items.map((item, idx) => (
                        <li key={idx} className="text-white/30 text-[10px] font-black flex items-center space-x-4 uppercase tracking-[0.2em] italic hover:text-primary transition-colors">
                  ...
                  <motion.a 
                    whileHover={{ x: 10 }}
                    href="#quote" 
                    className="btn-primary group/btn flex items-center justify-between px-12 py-6 rounded-[2rem]"
                  >
                    <span className="font-black italic tracking-[0.4em]">SPECIFICATIONS</span>
                    <ArrowRight size={20} className="group-hover/btn:translate-x-3 transition-transform duration-500 ease-apple" />
                  </motion.a>
                </div>
              </motion.div>
            ))}
        ...
        <section className="section-elite relative overflow-hidden bg-white/[0.01] border-y border-white/[0.03]">
          <div className="max-w-7xl mx-auto px-6">
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-24 gap-8">
              <motion.div {...fadeInUp}>
                <h2 className="text-[10px] font-black text-primary tracking-[0.5em] uppercase mb-8">REAL-TIME INVENTORY</h2>
                <h3 className="text-6xl md:text-7xl font-black text-white uppercase tracking-tighter italic leading-none">NEW <span className="text-white/10 uppercase">ARRIVALS</span></h3>
              </motion.div>
              <p className="text-white/20 text-[10px] font-black uppercase tracking-[0.4em] italic mb-4">Showing {products.length} Recent Additions</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12">
              {products.map((prod, i) => (
                <motion.div 
                  ...
                  {...fadeInUp}
                  transition={{ ...fadeInUp.transition, delay: i * 0.1 }}
                  className="bg-white/[0.02] rounded-[3rem] border border-white/5 overflow-hidden group hover:border-primary/20 transition-all duration-700 shadow-premium"
                >
                  <div className="aspect-square relative overflow-hidden">
                    ...
                    <div className="absolute top-8 right-8 px-6 py-2 glass-apple border-white/10 rounded-full opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all duration-700">
                      <span className="text-[9px] font-black text-primary uppercase tracking-[0.3em]">MOQ: {prod.MOQ}</span>
                    </div>
                  </div>
                  <div className="p-10">
                    <span className="text-[10px] font-black text-primary/40 uppercase tracking-[0.4em] italic leading-none">{prod.category}</span>
                    <h4 className="text-2xl font-black text-white uppercase mt-4 tracking-tighter italic leading-none truncate">{prod.title}</h4>
                    <p className="text-[10px] text-white/20 mt-6 line-clamp-2 uppercase font-black tracking-[0.2em] italic leading-relaxed">{prod.description}</p>
                    <div className="mt-10 h-[1px] w-12 bg-primary group-hover:w-full transition-all duration-1000 rounded-full"></div>
                  </div>
                </motion.div>
              ))}
            </div>
      ...
      <section className="section-elite relative overflow-hidden bg-background-dark">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-32 items-center">
          <motion.div {...fadeInUp}>
            <h2 className="text-[10px] font-black text-primary tracking-[0.5em] uppercase mb-8">INNOVATION HUB</h2>
            <h3 className="text-6xl md:text-7xl font-black text-white mb-16 uppercase leading-none tracking-tighter leading-[0.85]">FABRIC <br /><span className="text-white/10 italic">TECHNOLOGY</span></h3>
            <p className="text-xl text-white/30 leading-relaxed mb-20 font-medium italic">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-10">
              {[
                ...
                <motion.div 
                  ...
                  className="p-12 glass-apple rounded-[3.5rem] border border-white/5 hover:border-primary/20 transition-all duration-700 shadow-premium group"
                >
                  <div className="text-primary mb-8 bg-primary/10 w-16 h-16 rounded-3xl flex items-center justify-center border border-primary/20 shadow-glow group-hover:scale-110 transition-transform duration-700">
                    {tech.icon}
                  </div>
                  <h4 className="text-white font-black mb-4 uppercase tracking-[0.2em] text-lg italic leading-none">{tech.title}</h4>
                  <p className="text-white/20 text-[10px] uppercase font-black tracking-[0.3em] italic leading-relaxed">{tech.desc}</p>
                </motion.div>
              ))}
            </div>
          </motion.div>
          ...
          <motion.div 
            ...
            className="rounded-[4rem] overflow-hidden border border-white/5 relative group shadow-premium"
          >
            <img src="https://images.unsplash.com/photo-1620799140408-edc6dcb6d633?auto=format&fit=crop&q=80&w=1200" alt="SMI Fabric Technology" loading="lazy" decoding="async" className="w-full h-auto rounded-[3.9rem] opacity-40 grayscale group-hover:grayscale-0 group-hover:scale-105 transition-all duration-[2s] ease-apple" />
            <div className="absolute inset-0 bg-gradient-to-t from-background-dark via-transparent to-transparent" />
            <div className="absolute bottom-12 right-12 p-10 glass-apple rounded-[3rem] border border-white/10 text-center">
              <p className="text-primary font-black text-4xl leading-none italic uppercase tracking-tighter">HD-FIBER</p>
              <p className="text-[10px] font-black text-white/30 uppercase tracking-[0.5em] mt-3 italic">Technical Standard</p>
            </div>
          </motion.div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          >
            ...
            <div className="relative z-10">
              <h2 className="text-6xl md:text-8xl font-black text-white mb-16 uppercase italic leading-[0.85] tracking-tighter">READY TO DEFINE <br /><span className="text-primary text-glow">YOUR LINE?</span></h2>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-center space-y-8 sm:space-y-0 sm:space-x-12">
                ...
                >
                  <span className="tracking-[0.4em]">REQUEST QUOTATION</span>
